function [oCourse] = LoadTrack(strTrackName, oMainDialog)
    if ~isempty(strTrackName)
        ...
        switch oMainDialog.sSettings.convert
            case 'simple'
        ...
        switch oMainDialog.sSettings.filter
            case 'maf'
                n = 5; % iterations
                n_ma = 5; % weight-factor for conv
                strShape = 'valid'; % 'full' % 'same'
                for i = 1:n
                    x = Idealfinder.Lib.movingAverage(x, n_ma, strShape);
                    y = Idealfinder.Lib.movingAverage(y, n_ma, strShape);
                    z = Idealfinder.Lib.movingAverage(z, n_ma, strShape);
                end
            case 'dps'
                tol = 2; % [m] Abstand
